...
public class ResizingArrayStackWithReflection<Item> implements Iterable<Item> {
    ...
    public ResizingArrayStackWithReflection(Class<Item[]> itemArrayClass) {
        ...
        array = itemArrayClass
                .cast(Array.newInstance(itemArrayClass.getComponentType(), INIT_CAPACITY));
    }

    /**
     * Adds a non-{@code null} element on top of the Stack.
     *
     * @param item the element which is to be added to the Stack
    ...
    private void resize(int size) {
        Item[] newarray = itemArrayClass
                .cast(Array.newInstance(itemArrayClass.getComponentType(), size));
        for (int i = 0; i < n; i++)
            newarray[i] = array[i];
        array = newarray;
    }
    ...
    public static void main(String[] args) {
        ResizingArrayStackWithReflection<String> s = new ResizingArrayStackWithReflection<String>(
                String[].class);
        while (!StdIn.isEmpty()) {
